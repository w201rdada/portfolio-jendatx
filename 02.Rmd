# See Around Corners for Safety {#self-sensing-intersections-let-drivers-see-around-corners}
...
## Data Science for Informed Intersections
...
Once a pilot intersection is set up for detection and signal reception/connection, the Data Science team would supervise training of the smart intersection software so that the intersection can behave as a true node in the transit grid. The software would need to be able to translate detected momentum patterns into risk categories (identifying, for instance, everything from a 🐈 or 🛴 or 🚴‍♀️ or 🚚, etc.). Ground truth could be provided for training by implementing temporary detours while testers use the intersection via all common routes and methods.  
